...
;( function ( $, window, undefined ) {
    ...
    Plugin.prototype.handlerErrorRequest = function ( result_data ) {
        var self = this;
    ...
    Plugin.prototype.parseTemplate = function ( repo ) {
        ...
        repo.parseDate   = this.parsePushedDate( repo.pushed_at );
        repo.parseUrl    = self.parseRepositoryURL( repo.url );
        
        var $template   = $.parseHTML(" \
                            <div class='github-box'>  \
                                <div class='github-box-header'> \
                                    <h3> \
                                        <a href='" + repo.parseUrl + "'>" + repo.name + "</a> \
                                    </h3> \
                                    <div class='github-stats'> \
                                        <a class='repo-watchers' href='" + repo.parseUrl + "/watchers'>" + repo.watchers + "</a> \
                                        <a class='repo-forks' href='" + repo.parseUrl + "/network'>" + repo.forks + "</a> \
                                    </div> \
                                </div> \
                                <div class='github-box-content'> \
                                    <p>" + repo.description + " &mdash; <a href='" + repo.parseUrl + "#readme'>Read More</a></p> \
                                </div> \
                                <div class='github-box-download'> \
                                    <p class='repo-update'>Latest commit to <strong>master</strong> on " + repo.parseDate + "</p> \
                                    <a class='repo-download' href='" + repo.parseUrl + "/zipball/master'>Download as zip</a> \
                                </div> \
                            </div> \
                          ");

        return $( self.options.template( repo ) || $template );
    };
    ...
    Plugin.prototype.requestData = function ( repo ) {
        ...
        $.ajax({
            ...
            beforeSend: function() {
                ...
                self.$container.html( self.options.loader() );
            ...
            success: function( results ) {
                ...
                if ( results.meta.status >= 400 && result_data.message ) {
                    ...
                    self.handlerErrorRequest( results.meta.status );
